...
const vm = new Vue({
  ...
  methods: {
    ...
    publish: async function(screenSharing = false) {
      ...
      this.localStream = localTrack.stream; // Lưu localStream
    ...
    toggleCamera() {
      if (this.localStream) {
        ...
        if (videoTracks.length > 0) {
          this.isCameraOn = !this.isCameraOn; // Đảo ngược trạng thái camera
          videoTracks[0].enabled = this.isCameraOn; // Tắt/mở camera
        }
      }
    }
  }
